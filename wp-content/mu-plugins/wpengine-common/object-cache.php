...
class WP_Object_Cache {
    ...
    function key($key, $group) {
        ...
        // JC: str_replace is faster when you don't need a regex
        $bucket = $this->bucket_for_group($group);
        $generation = $this->get_generation($bucket);
        return str_replace(' ', '', "v1:$prefix$group:$key:$generation");
    }
    ...
    function WP_Object_Cache() {
        ...
        $customer = preg_match( "#^/nas/wp/www/(?:cluster-\\d+|sites)/([^/]+)/#", __FILE__, $match ) ? $match[1] : md5( __FILE__ );
